...
MODEL_PATH = "out/kitti_360/s4c_base/kitti_360_backend-None-1_20231220-233015" # 10:40 weak weights converged
...
PLY_PATH = Path("/data/GPT/s4c-main/voxel_output/s4c")
...
def main():
    ...
    for i in pbar:
        ...
        if GENERATE_PLY_FILES:
            ...
            folder_path = PLY_PATH / f"{id:06d}"
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)

            for size in PLY_SIZES:
                if not os.path.exists(folder_path / str(int(size))):
                    os.makedirs(folder_path / str(int(size)))

            for size in PLY_SIZES:
                num_voxels = int(size // 0.2)
                save_as_voxel_ply(folder_path / str(int(size)) / f"{id:06d}.ply",
                ...
                if target is not None:
                    save_as_voxel_ply(folder_path / str(int(size)) / f"{id:06d}_gt.ply",
            ...
            image_numpy = data['imgs'][0].permute(1,2,0).cpu().numpy()
            ...
            cv2.imwrite(os.path.join(folder_path, f"{id:06d}_img.png"), img_bgr)

...
class Kitti360Dataset(Dataset):
    def __init__(self,
                 ...
                 data_depth_path=None,
                 data_depth_std_path=None,
                 data_normal_path=None,
                 frame_count=2,
                 keyframe_offset=0,
                 dilation=1,
                 fisheye_rotation=0,
        ...
        self.data_depth_path = data_depth_path
        self.data_depth_std_path = data_depth_std_path
        self.data_normal_path = data_normal_path
        ...
        self._depth_perspective_folder = "data_192x640"
        self._depth_fisheye_folder = "data_192x640_0x-15"
    ...
    def load_depth_images(self, seq, img_ids, load_left, load_right, img_ids_fish=None):
        imgs_p_left = []
        imgs_f_left = []
        imgs_p_right = []
        imgs_f_right = []
        ...
        for id in img_ids:
            if load_left:
                img_perspective = self.load_depth_from_path(os.path.join(self.data_depth_path, seq, "image_00", self._depth_perspective_folder, f"{id:010d}.png"))
                imgs_p_left += [img_perspective]

            if load_right:
                img_perspective = self.load_depth_from_path(os.path.join(self.data_depth_path, seq, "image_01", self._depth_perspective_folder, f"{id:010d}.png"))
                imgs_p_right += [img_perspective]

        for id in img_ids_fish:
            if load_left:
                img_fisheye = self.load_depth_from_path(os.path.join(self.data_depth_path, seq, "image_02", self._depth_fisheye_folder, f"{id:010d}.png"))
                imgs_f_left += [img_fisheye]
            if load_right:
                img_fisheye = self.load_depth_from_path(os.path.join(self.data_depth_path, seq, "image_03", self._depth_fisheye_folder, f"{id:010d}.png"))
                imgs_f_right += [img_fisheye]

        depth = imgs_p_left + imgs_p_right + imgs_f_left + imgs_f_right
        return depth


    def load_depth_std_images(self, seq, img_ids, load_left, load_right, img_ids_fish=None):
        imgs_p_left = []
        imgs_f_left = []
        imgs_p_right = []
        imgs_f_right = []
        ...
        for id in img_ids:
            if load_left:
                img_perspective = self.load_depth_std_from_path(os.path.join(self.data_depth_std_path, seq, "image_00", self._depth_perspective_folder, f"{id:010d}.png"))
                imgs_p_left += [img_perspective]

            if load_right:
                img_perspective = self.load_depth_std_from_path(os.path.join(self.data_depth_std_path, seq, "image_01", self._depth_perspective_folder, f"{id:010d}.png"))
                imgs_p_right += [img_perspective]

        for id in img_ids_fish:
            if load_left:
                img_fisheye = self.load_depth_std_from_path(os.path.join(self.data_depth_std_path, seq, "image_02", self._depth_fisheye_folder, f"{id:010d}.png"))
                imgs_f_left += [img_fisheye]
            if load_right:
                img_fisheye = self.load_depth_std_from_path(os.path.join(self.data_depth_std_path, seq, "image_03", self._depth_fisheye_folder, f"{id:010d}.png"))
                imgs_f_right += [img_fisheye]

        depth_std = imgs_p_left + imgs_p_right + imgs_f_left + imgs_f_right

        return depth_std

    def load_normal_images(self, seq, img_ids, load_left, load_right, img_ids_fish=None):
        imgs_p_left = []
        imgs_f_left = []
        imgs_p_right = []
        imgs_f_right = []
        ...
        for id in img_ids:
            if load_left:
                img_perspective = self.load_normal_from_path(os.path.join(self.data_normal_path, seq, "image_00", self._depth_perspective_folder, f"{id:010d}.png"))
                imgs_p_left += [img_perspective]

            if load_right:
                img_perspective = self.load_normal_from_path(os.path.join(self.data_normal_path, seq, "image_01", self._depth_perspective_folder, f"{id:010d}.png"))
                imgs_p_right += [img_perspective]

        for id in img_ids_fish:
            if load_left:
                img_fisheye = self.load_normal_from_path(os.path.join(self.data_normal_path, seq, "image_02", self._depth_fisheye_folder, f"{id:010d}.png"))
                imgs_f_left += [img_fisheye]
            if load_right:
                img_fisheye = self.load_normal_from_path(os.path.join(self.data_normal_path, seq, "image_03", self._depth_fisheye_folder, f"{id:010d}.png"))
                imgs_f_right += [img_fisheye]

        normal = imgs_p_left + imgs_p_right + imgs_f_left + imgs_f_right
        return normal
    ...
    def load_segmentation_from_path(self, path):
    ...
    def load_depth_from_path(self, path):
        depth = cv2.imread(path, cv2.IMREAD_UNCHANGED).astype(np.float32) / 500.0
        depth = cv2.resize(depth, (self.target_image_size[1], self.target_image_size[0]), interpolation=cv2.INTER_NEAREST)
        return depth

    def load_depth_std_from_path(self, path):
        depth_std = cv2.imread(path, cv2.IMREAD_UNCHANGED).astype(np.float32) / 500.0
        depth_std = cv2.resize(depth_std, (self.target_image_size[1], self.target_image_size[0]), interpolation=cv2.INTER_NEAREST)
        return depth_std

    def load_normal_from_path(self, path):
        normal = cv2.imread(path, cv2.IMREAD_UNCHANGED).astype(np.float32) / 255.0 * 2.0 -1.0
        normal = cv2.resize(normal, (self.target_image_size[1], self.target_image_size[0]), interpolation=cv2.INTER_NEAREST)
        return normal
    ...
    def __getitem__(self, index: int):
        ...
        depth_image = []
        depth_std_image = []
        if self.depth_mode == 'spi_depth':
            depth_image = self.load_depth_images(sequence, img_ids, load_left, load_right, img_ids_fish=img_ids_fish)
            depth_std_image = self.load_depth_std_images(sequence, img_ids, load_left, load_right, img_ids_fish=img_ids_fish)

        normal_image = []
        if self.normal_mode == 'normal':
            normal_image = self.load_normal_images(sequence, img_ids, load_left, load_right, img_ids_fish=img_ids_fish)

        # Load the kitti-360 gt
        segs_kitti_gt = torch.zeros(0)
        if self.load_kitti_360_segmentation_gt:  # False
            segs_raw = [self.load_segmentation(sequence, id) for id in img_ids]
        ...
        data = {
            ...
            "depth_gt_map": depth_image,
            "depth_std_gt_map": depth_std_image,
            "normal_gt_map": normal_image,
